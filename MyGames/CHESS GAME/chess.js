  ...
  $('.chess_piece').click(function(e) {
   ...
   if(global.variables.game_end==true){
    ...
    if(selectedpiece.name.slice(0,1)=='w'){
      ...
      $(".player1").css("background-color","lightcoral")
    }else if(selectedpiece.name.slice(0,1)=='b'){
      $(".player2").html("YOUR KING IS ON CHECK");
      $(".player2").css("background-color","lightcoral")
    }
    
    global.variables.check=false;

   } else  if (global.variables.selectedpiece == '' && selectedpiece.name.slice(0,1) == global.variables.turn) {
...
//it is first function to execute when page load
   ...
   //forCheck argument used for if options are highlight or return
             ...
             if (single_move[0] < start_pos1 || single_move[0] > start_pos1){ 
              ...
              if ($('#' + allmoves[i]).attr('chess') != 'null' && ($('#' + allmoves[i]).attr('chess')).slice(0,1) == 'b'){
               ...
               if($('#' + allmoves[i]).attr('chess') != 'null'){
                w_pawn_flag=true;  //if this is the pawns first move, and there is a piece in front ;
               }
               else if ($('#' + allmoves[i]).attr('chess') == 'null'){
                  if(w_pawn_flag==false){
        ...
        case 'b_pawn':
          ...
          //check if it has an opponent piece on it;
           if (single_move[0] < start_pos1 || single_move[0] > start_pos1){ 
            ...
            if($('#' + allmoves[i]).attr('chess') != 'null'){
              b_pawn_flag=true;      //if this is the pawns first move, and there is a piece in front ;
           }
          else if ($('#' + allmoves[i]).attr('chess') == 'null'){

            if(b_pawn_flag==false){
              possible_moves.push(allmoves[i]);     // otherwise return the move if there is no chess piece on it;
                 }
              }   
            }
    ...
    function move(target) {
     ...
     global.variables.recent_move_attr.push(selectedpiece,targetpiece);
      ...
      //set the target id is selectedpiece id 
      global.variables.selectedpiece=target.id;
      Check(selectedpiece);
    }
    ...
    function capture (target) {
     ...
     if(global.variables.pieces[target.name].type.slice(0,1)=='w'){
      ...
      if(global.variables.pieces[target.name].type=='b_king'){
        $("#result").show();
        $("#result").html("PLAYER 1 WINS");
        $(".game_board").css("opacity","0.2");
        $(".captured_icons").css("visibility","hidden");
        global.variables.game_end=true;
      }else if(global.variables.pieces[target.name].type=='w_king'){
        $("#result").show();
  ...
  function endturn(){
   ...
   //function to set check or not
   
  function Check(selectedpiece){

    options=[];

    if(selectedpiece.slice(0,1)=='w'){
      options=w_show_moves(selectedpiece,true);
    }else if(selectedpiece.slice(0,1)=='b'){
      options=b_show_moves(selectedpiece,true);
  }
    togglehighlight(global.variables.highlighted);
    if(selectedpiece.slice(0,1)=='w'){
      for(let i=0;i<options.length;i++){
             if(($("#"+options[i]).attr('chess'))=='b_king'){
                 global.variables.check=true;
             }
      }
    }else if(selectedpiece.slice(0,1)=='b'){
     for(let i=0;i<options.length;i++){
            if($("#"+options[i]).attr('chess')=='w_king'){
                global.variables.check=true;
            }
     }
   }
