...
public class StudentPassChange extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Cookie c:cookies){
			if(c.getName().equals("student_forgot_password_key")){
		...
		if(forgot_password_key!="") {
			try{  
				...
				PreparedStatement ps=con.prepareStatement("update student_details set password=? where forgot_password_key=?");
				ps.setString(1, newpassword);
				ps.setString(2,forgot_password_key);
				int execution=ps.executeUpdate();   //execute the command to change password
				if(execution!=0) {
					Cookie cookie=new Cookie("forgot_password_key","");
				    cookie.setMaxAge(0);
